...
const UsersTask: React.FC = (): JSX.Element => {
  ...
  const { tasks, isAuth, updateCount, countPageTask } = useTypedSelector(
    (state) => state.usersTask,
  )
  ...
  const { logOut, updateUserStatus, deleteData, getCount, getTask } =
    useActions()
      ...
      <div className="container flex justify-center">
        <div className="w-full  max-w-md min-w-80 px-2  h-full flex flex-col">
          <div className="px-4 container  flex justify-between items-center py-2 shadow border-b border-gray-300">
            <div className=" bg-white text-sm text-gray-500 font-bold ">
              Тasks
            </div>
            <SortDropdown />
          </div>
          <div className="w-full h-full overflow-auto shadow bg-white">
            <table className="w-full">
              <tbody className="">
                {tasks.map(
                  ({ email, name, task, time, _id, isDone, edited }) => (
                    <tr
                      key={_id}
                    ...
                    >
                      <td className="pl-3 pr-3 min-w-max whitespace-no-wrap">
                        ...
                        {edited && (
                          <div className="text-red-400  pb-1">edited</div>
                        )}
                      </td>
                      <td className="px-2 py-4 whitespace-no-wrap">
                        ...
                        <div className="leading-5 text-xs text-gray-900">
                          {task}
                        </div>
                        {isAuth && (
